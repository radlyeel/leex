#include "io.h"
//TODO: unify names to snake case.
// GPIO

// See BCM2711 ARM Peripherals Sec. 5.2
enum { // Where is the explanation for this base address?
    PERIPHERAL_BASE = 0xFE000000, // 
    GPFSEL0         = PERIPHERAL_BASE + 0x200000,
    GPSET0          = PERIPHERAL_BASE + 0x20001C,
    GPCLR0          = PERIPHERAL_BASE + 0x200028,
    GPPUPDN0        = PERIPHERAL_BASE + 0x2000E4,
};
...
unsigned int gpio_pull    (unsigned int pin_number, unsigned int value) { 
    return gpio_call(pin_number, value, GPPUPDN0, 2, GPIO_MAX_PIN); 
}
...
void uart_write_text(const char *buffer) { while (*buffer) { if (*buffer ==
